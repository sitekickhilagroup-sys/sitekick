...
function fakeAdmin(existing: { id: string } | null) {
...
describe('ingestDocument', () => {
  it('dedups on external_id', async () => {
    const { admin, inserted } = fakeAdmin({ id: 'doc-existing' });
    const result = await ingestDocument(admin, {
      kind: 'email', source: 'forward', external_id: 'msg-1', raw_text: 'x',
    });
    expect(result).toEqual({ documentId: 'doc-existing', deduped: true });
  ...
  it('inserts new document when unseen', async () => {
    ...
    expect(result).toEqual({ documentId: 'new-doc', deduped: false });
