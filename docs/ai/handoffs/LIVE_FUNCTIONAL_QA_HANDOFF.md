...
5. **Failure-counter semantics** ("first or second attempt stays Waiting, only the third becomes Failed"): confirmed already correct by construction — `selectPermanentlyFailed`'s existing unit test asserts exactly this (a count of 2 stays out of the failed set, a count of 3 enters it, with `MAX_ATTEMPTS=3`) — **CODE-VERIFIED**, not forced live since manufacturing a real failure would mean deliberately breaking a real document's extraction.

**Known gap, documented not silently skipped:** the cron's schedule (once daily) hasn't been live-verified as actually firing — Vercel's cron execution isn't something this session can trigger on demand or observe outside a real scheduled tick, and the account's plan-tier cron limits are unknown, which is exactly why the schedule was kept conservative rather than guessed at aggressively. The manual button above is the proven, immediately-usable path to keep draining the remaining ~171 waiting documents (366 stored / 195 processed / 171 waiting / 0 failed as of this line).
