...
export function CompareCharts({ openMoney, tasks, projectNames, title, moneyLabel, loadLabel, allLabel }: Props) {
  ...
  for (const t of tasks) {
    ...
    const name = (t.project_id ? projectNames[t.project_id] : null) ?? allLabel;
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        <div className="rounded-(--radius-card) border border-line bg-card p-4 shadow-card">
          ...
          <div className="mt-3 space-y-2">
            {openMoney.map((m) => (
              <div key={m.project ?? '__all__'} className="grid grid-cols-[130px_1fr_90px] items-center gap-2 text-xs">
                <span className="truncate text-ink2">{m.project ?? allLabel}</span>
